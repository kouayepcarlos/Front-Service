...
export const authAPIadmin = {
  ...
  addSubject: async (credentials) => {
    ...
    try {
      ...
      if (credentials.selectedOption == 1) {
        ...
        response = await API.post("/admins/chargerSujetExamen", formData, {
          headers: {
        ...
        response = await API.post("/admins/chargerSujetUniversite", formData, {
          headers: {
            "Content-Type": "multipart/form-data",
            Authorization: `Bearer ${token}`,
          },
  ...
  updateAdmin: async (credentials) => {
    ...
    try {
      const response = await API.put(
        ...
        {
          username: credentials.nom == null ? null : credentials.nom.trim(),
          email: credentials.email == null ? null : credentials.email.trim(),
          password: credentials.passe == null ? null : credentials.passe.trim(),
        },
        {
          headers: {
            "Content-Type": "application/json",
  ...
  deleteremessage: async (credentials) => {
    ...
    // eslint-disable-next-line no-useless-catch
    try {
      const response = await API.delete(`admins/contacts/${credentials.id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
