...
export function AppProvider({ children }) {
   ...
   */
   ...
   const { data: sessionUser, isError:errorUser, refetch:refetchSession, isLoading:loadingSession } = useQuery({
    queryKey: ["session"],
  ...
  const { data: listeSujets, refetch: refetchSujets } = useQuery({
    ...
    // Récupération des sujets d'examen
  const { data: listeSujetsUniversites, refetch: refetchSujetsUniversites } = useQuery({
  ...
  useEffect(() => {
    const checkSessionInterval = setInterval(() => {
      if (authAPI.isAuthenticated() && authAPI.isSessionExpired(SESSION_TIMEOUT)) {
...
const downloadSubjetMutation = useMutation({
  mutationFn: async (id) =>
      await authAPI.downloadSubjet(id),
...
const downloadSubjetUniversiteMutation = useMutation({
  mutationFn: async (id) =>
      await authAPI.downloadSubjetUniversite(id),
  ...
  if (!isInitialized && !publicRoutes.includes(location.pathname)) {
    return <div>
        <LoaderTransparent/>
    </div>; // Ou un composant de chargement plus élaboré
  }
