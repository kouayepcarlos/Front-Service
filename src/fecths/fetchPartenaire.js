...
export const authAPIPartenaire = {
  ...
  completedAccount: async (credentials) => {
    ...
    try {
      ...
      const response = await API.post("/partenaire/completeAccount", formData, {
        headers: {
  ...
  nouvelAbonnement: async (credentials) => {
    ...
    try {
      console.log(credentials);
      const response = await API.post("partenaire/abonnement", credentials, {
        headers: {
          Authorization: `Bearer ${token}`, // Ajoute le token dans les headers
        },
      });
  ...
  getObjectifs: async () => {
    ...
    try {
      ...
      console.log(res.data);
      return res.data.liste_des_objectifs;
    } catch (error) {
      console.error("Erreur lors de la récupération des sujets :", error);
      return {
        status: "error",
